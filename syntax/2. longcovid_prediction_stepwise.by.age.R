...
summary(train)
train <- train[,-c("COVID.test","major","ild","ards","inpatient","ageg","pd","bp","anxiety","htn","cbc.wbc","cbc.neuphil","cbc.plt","inflam.crp","inflam.ldh")]
...
save(model.train,file="age18-44.BW.Rdata")
...
model.train <- stepwiseLogit(as.formula(paste0("death_1y~",paste(var,collapse = "+"))),data = train,
                             selection = "backward",select = "SBC")
save(model.train,file="age45-64.BW.Rdata")
...
var <- names(train)
var <- var[!var %in% c("id","death_1y","Age")]
model.train <- stepwiseLogit(as.formula(paste0("death_1y~",paste(var,collapse = "+"))),data = train,
                             selection = "backward",select = "SBC")
save(model.train,file="age65.BW.Rdata")
load("age65.BW.Rdata")
v <- as.data.frame(t(model.train$`Selected Varaibles`))
v1 <- v[v$xModel!=1,]
var <- paste(v1, collapse = "+")
...
perf.f <- function(data, indices){
  ...
  pred_.boot2 <- as.factor(ifelse(predict(model.OR, new.data, type="response")>0.02,"1","0"))
  ...
  pred_.boot4 <- as.factor(ifelse(predict(model.OR, new.data, type="response")>0.04,"1","0"))
