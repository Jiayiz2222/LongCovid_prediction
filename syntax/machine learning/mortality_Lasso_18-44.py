...
RANDOM_STATE = 0
cohort_train = pd.read_csv('../dataset/train.18-44.csv')
cohort_test = pd.read_csv('../dataset/test.18-44.csv')
deleted_var = ["id", "major", "ards", "ageg", "pd", "bp", "anxiety", "htn", "cbc.wbc", "cbc.neuphil", "cbc.plt", "inflam.crp", "inflam.ldh"]
...
if __name__=='__main__':
    ...
    grid_search = GridSearchCV(estimator=lasso, param_grid=param_grid, scoring='roc_auc', cv=10, n_jobs=-1, verbose=0)
    grid_search.fit(X_train[selected_variables], y_train)
    print('Tuned Hyperparameters:', grid_search.best_params_)
    model = LogisticRegression(C=grid_search.best_params_['C'], penalty='l1', solver='liblinear', random_state=RANDOM_STATE)
    ...
    pd.DataFrame({'True': y_train, 'Predicted': y_score_train}).to_csv('../predicted_proba_train/predicted_proba_18-44/' + method + '.csv', header=False, index=False)
    y_score_test = model.predict_proba(X_test)
    y_score_test = pd.Series(y_score_test[:,1])
    pd.DataFrame({'True': y_test, 'Predicted': y_score_test}).to_csv('../predicted_proba_test/predicted_proba_18-44/' + method + '.csv', header=False, index=False)
