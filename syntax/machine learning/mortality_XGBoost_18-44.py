...
if __name__=='__main__':
    ...
    pd.DataFrame({'True': y_train, 'Predicted': y_score_train}).to_csv('../predicted_proba_train/predicted_proba/' + method + '.csv', header=False, index=False)
    y_score_test = model.predict_proba(X_test[selected_variables])
    y_score_test = pd.Series(y_score_test[:,1])
    pd.DataFrame({'True': y_test, 'Predicted': y_score_test}).to_csv('../predicted_proba_test/predicted_proba/' + method + '.csv', header=False, index=False)
