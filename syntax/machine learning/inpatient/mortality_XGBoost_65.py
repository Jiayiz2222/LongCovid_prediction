...
RANDOM_STATE = 0
cohort_train = pd.read_csv('../../dataset/inpatient/train_123.inpatient_65.csv')
cohort_test = pd.read_csv('../../dataset/inpatient/test_123.inpatient_65.csv')
...
pd.DataFrame({'True': y_train, 'Predicted': y_score_train}).to_csv('../../predicted_proba_train/inpatient/predicted_proba_65/' + method + '.csv', header=False, index=False)
y_score_test = model.predict_proba(X_test[selected_variables])
y_score_test = pd.Series(y_score_test[:,1])
pd.DataFrame({'True': y_test, 'Predicted': y_score_test}).to_csv('../../predicted_proba_test/inpatient/predicted_proba_65/' + method + '.csv', header=False, index=False)
